    ...
    const fetchCoinData = async () => {
      try {
        ...
        for (let i = 0; i < response.data.prices.length; i++) {
          ...
          dataArry.push({
            date: new Date(response.data.prices[i][0]).toLocaleDateString(),
            price: price < "50" ? price : parseInt(price),
          });
        }
        setCoinData(dataArry);
      ...
      >
        <defs>
          <linearGradient id="colorUv" x1="0" y1="0" x2="0" y2="1">
            <stop offset="7%" stopColor={styles.color1} stopOpacity={0.8} />
            <stop offset="95%" stopColor={styles.white1} stopOpacity={0} />
          </linearGradient>
        </defs>
        <XAxis dataKey="date" />
